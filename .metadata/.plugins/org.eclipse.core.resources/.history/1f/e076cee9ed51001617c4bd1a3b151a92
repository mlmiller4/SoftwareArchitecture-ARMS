			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!textFieldCourse1.getText().isEmpty()) {
					...
					if (!textFieldCourse2.getText().isEmpty()) {
						...
						courseIDs.add(Integer.parseInt(textFieldCourse2
								.getText()));

						if (!textFieldCourse3.getText().isEmpty()) {
							courses += ", " + textFieldCourse3.getText();
							courseIDs.add(Integer.parseInt(textFieldCourse3
									.getText()));

							if (!textFieldCourse4.getText().isEmpty()) {
								courses += ", " + textFieldCourse4.getText();
								courseIDs.add(Integer.parseInt(textFieldCourse4
										.getText()));

								if (!textFieldCourse5.getText().isEmpty()) {
									courses += ", "
											+ textFieldCourse5.getText();
									courseIDs.add(Integer
											.parseInt(textFieldCourse5
													.getText()));
								}
							}
						}
					...
					// What parameters are needed here?? The 'ScheduleRequests'
					// table only has fields: StudentID, SRId, SubmitTime

					// dbActions.InsertSchedule(gtId, CourseId, Semester,
					// schedType)
